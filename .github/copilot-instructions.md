...
Quick orientation (what matters): this is a Next.js (App Router) TypeScript app using Supabase for auth/data and Tailwind/UI primitives under `src/`. The project uses Turbopack in development and `pnpm` as the package manager.

Key workflows
- Development server: `pnpm dev` (or `npm run dev`). Note: `dev` runs `next dev --turbopack` with `NODE_OPTIONS='--inspect'` (inspect/debug enabled).
